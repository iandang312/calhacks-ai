...
class DaisyService : android.app.Service() {
        ...
        when (command.action) {
            ...
            AgentAction.SCAN_SCREEN -> {
                val visibleText = DaisyAccessibilityService.scanScreen()
                    ...
                    .take(6)
                speak(
                    if (visibleText.isEmpty()) "I don't see readable controls on this screen."
                    else "I see ${visibleText.joinToString(", ")}.",
                )
            }
            AgentAction.GO_BACK -> {
    ...
    private fun handleDrag(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                hideDismissTarget()
                if (shouldDismiss) removeOverlay()
                return true
            }
        }
        return false
    ...
    companion object {
        private const val CHANNEL_ID = "echomind_service"
